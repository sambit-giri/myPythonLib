...
def fof(data, xth=0.5, boxsize=100):
	...
	fofbubble.fof(data, xth, boxsize)
	f = np.loadtxt('dummy_output_of_fof.dat')
	...
	for i in xrange(len(names)):
		row = np.array(names[i].split())
		row.astype(np.float)
		nbubble.append(row[0])
		volume.append(row[1])
		radius.append(row[2])
	
	nbubble = np.array(nbubble)
...
def zahn(data, xth=0.5, boxsize=100, nscales=20, upper_lim=False):
	...
	zahnbubble.zahn(data, xth, boxsize, nscales)
...
def mfp(data, xth=0.5, boxsize=100, iterations = 10000000, verbose=True, upper_lim=False):
	...
	if dim == 2:
		...
		out = mfp2d(data, xth, boxsize=boxsize, iterations=iterations, verbose=verbose)
	elif dim == 3:
		print "MFP method applied on 3D data"
		out = mfp3d(data, xth, boxsize=boxsize, iterations=iterations, verbose=verbose)
	else:
